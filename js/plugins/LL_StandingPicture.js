...
(() => {
	...
	const sPictureId = param.sPictureId;
	const transition = param.transition;
	...
	const showStandingPicture = (picture) => {
		...
		// 表示する立ち絵の情報を取得
		var pictureName = picture.imageName;
		var origin = picture.origin;
		var x = picture.x;
		var y = picture.y;
		var scaleX = picture.scaleX;
		var scaleY = picture.scaleY;
		var opacity = picture.opacity;
		var blendMode = picture.blendMode;
		var pictureTone = $gameSystem._StandingPictureTone ? $gameSystem._StandingPictureTone : [0, 0, 0, 0];
		// 画像ファイル存在チェック
		var picturePath = 'img/pictures/' + pictureName + '.png';
		var img = new Image();
		img.onload = function() {
			// ピクチャ表示
			if (!activePicture) {
	...
	const hideStandingPicture = () => {
		...
		// ピクチャ消去
		if (activePicture) {
			$gameScreen.movePicture(sPictureId, activePicture.origin, activePicture.x + startX, activePicture.y + startY, activePicture.scaleX, activePicture.scaleY, 0, activePicture.blendMode, duration, 0);
			activePicture= null;
		}
	};
	...
	Window_Message.prototype.updateClose = function() {		
	    if (this._closing) {
	    	// ピクチャ削除判定
	    	//if (this.openness == 255 && !$gameParty.inBattle()) {
	...
	Window_Message.prototype.startMessage = function() {
		//if (!$gameParty.inBattle()) {
			...
			var processEscapeNumber = $gameMessage.allText().match(/\\F\[(\d+)\]/);
			...
			// 立ち絵を更新
			if (sPictureNumber) {
				var sPicture = param.sPictures.filter(function(item, index) {
