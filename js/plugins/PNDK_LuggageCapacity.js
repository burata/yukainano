 ...
 *   PNDK_LC callCommonId [common ID]
 *
 * Disclaimer:
 ...
 * ・重量オーバーしているか確認し、重量オーバーしている場合はID2のコモンが
 * 　呼ばれるコマンドです。
 *   PNDK_LC callCommonId 2
 * 
 * ■総重量ウィンドウの位置について
		...
		if (command === 'PNDK_LC') {
			switch (args[0]) {
				...
				case 'checkWeights':
					...
					break;
				// Add end by nyatama
				// Add start by mai 2020/06/08
				case 'callCommonId':
					if (getSurplusCP() < 0) $gameTemp.reserveCommonEvent(args[1]);
					break;
				// Add end by mai
			}
		}
